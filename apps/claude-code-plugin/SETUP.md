...
3a. SOURCE path (global install):
    - pip install -e . 'powermem[server]' 'powermem[mcp]'
      ⚠️ Both extras are required: `[server]` adds fastapi/uvicorn; `[mcp]` adds
...
**Fix**:
```bash
pip install 'powermem[server]'   # fastapi + uvicorn
pip install 'powermem[mcp]'      # fastmcp (required even in HTTP-only mode)
```
